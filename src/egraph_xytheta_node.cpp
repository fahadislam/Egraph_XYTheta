...
EGraphXYThetaNode::EGraphXYThetaNode(costmap_2d::Costmap2DROS* costmap_ros) {
  ...
  env_ = new EGraphXYTheta(vis_pub);
  ...
  //fahad
  planner_ = new MHAPlanner(env_, 2, true, true);
...
bool EGraphXYThetaNode::makePlan(egraph_xytheta::GetXYThetaPlan::Request& req, egraph_xytheta::GetXYThetaPlan::Response& res){
  ...
  //fahad
    MHAReplanParams replan_params(60);
    ...
    int soln_cost;
    int ret = planner_->replan(&solution_stateIDs, replan_params, &soln_cost);
    //
  ...
  packageMHAStats(stat_names, stats, soln_cost, solution_stateIDs.size());
  // m_stats_writer.writeSBPL(stats, states, counter, planner_prefix);
  res.stat_names = stat_names;
  res.stat_values = stats;
...
void EGraphXYThetaNode::packageMHAStats(vector<string> &stat_names,
vector<double> &stats,
int solution_cost,
